...
func GoGen(dir string) {
	...
	r := mkRewriter(matcher.NewMatcher(
		...
		matcher.WithBuildTag("co"),
		matcher.WithFileFilter(func(filename string, file *ast.File) bool {
			return strings.HasSuffix(filename, "_co.go") ||
				strings.HasSuffix(filename, "_co_test.go")
		}),
	))
	r.rewriteAllFiles(func(filename string, file *ast.File) {
		filename = strings.ReplaceAll(filename, "_co.go", ".go")
		filename = strings.ReplaceAll(filename, "_co_test.go", "_test.go")
